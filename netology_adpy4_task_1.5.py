...
class MailProcessor:
    ...
    def __init__(self, mail_smtp, mail_smtp_port, mail_imap, mail_imap_port, login, password):
    ...
    def send_email(self, subject, message):
        recipients = input('Введите адресатов (через запятую без пробелов): ') \
            .lower().split(',')
        ...
        mail_client.sendmail(self.login, send_email['To'].split(', '), send_email.as_string())
...
if __name__ == '__main__':
    gmail = MailProcessor(mail_smtp, mail_smtp_port, mail_imap, mail_imap_port, login, password)
    gmail.send_email()
